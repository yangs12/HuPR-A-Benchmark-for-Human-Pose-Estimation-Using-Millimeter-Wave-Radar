...
class HuPR3D_horivert(BaseDataset):
    def __init__(self, phase, cfg, args, random=True):
        ...
        for name in self.imageIds:
            namestr = '%09d' % name
            self.folders.append(int(namestr[:4]))
    ...
    def __getitem__(self, index):
        ...
        # print('torch', torch.is_tensor(VRDAEmaps_hori))
        ...
        # np.save('hupr_data_test/VRDAEmaps_hori_'+ str(folder)+'_' + str(file_name)+ '.npy', VRDAEmaps_hori)
        # np.save('hupr_data_test/VRDAEmaps_vert_'+ str(folder)+'_' + str(file_name)+ '.npy', VRDAEmaps_vert)
        # print('saved')
